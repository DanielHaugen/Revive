...
| 5.1.3 | Audit log API routes (list, filter, paginate) | ✅ Done | `app/api/logs/route.ts` — GET with page/pageSize/action/resourceId filters. | PRD §3.8 |
| 5.1.4 | Audit log page with filterable DataTable | ✅ Done | `app/(routes)/logs/page.tsx` — DataTable with action/resource filters, pagination. | PRD §3.8 |
...
| 5.2.6 | Restore history page (`/restoration`) | 🔲 Todo | Replaces the current placeholder on `/restoration`. Table of all completed restores: machine, snapshot used, TTR, triggered by (user or playbook), session. Filterable by session and machine. | PRD §3.5 |
