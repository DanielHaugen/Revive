...
import { awsInstanceIdSchema, resourceTagsSchema } from '@/lib/validation/schemas';

export async function PUT(
  req: Request,
  { params }: { params: { id: string } }
) {
  const paramResult = validateParam(params.id, awsInstanceIdSchema);
  ...
  try {
    ...
    syncInstances().catch((e) => console.error('Post-tag instance sync failed:', e));
